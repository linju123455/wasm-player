<template>
    <canvas id="scene_base" ref="scene_base" :width="width" :height="height" style="background-color:#000;"></canvas>
    <!-- <canvas id="scene_ext" ref="scene_ext" :width="width" :height="height"></canvas> -->
</template>
...
export default {
    ...
    methods: {
        initGL() {
            var canvas = this.$refs["scene_base"];
            this.gl = canvas.getContext('webgl');

            let vertexShader = this.initVertexShader();
            let fragmentShader = this.initFragmentShader();
            let program = this._createProgram(vertexShader, fragmentShader);
            this._initVertexBuffers(program);

            // 激活指定的纹理单元
            this.gl.activeTexture(this.gl.TEXTURE0);
            this.gl.y = this._createTexture();
            this.gl.uniform1i(this.gl.getUniformLocation(program, 'samplerY'), 0);

            this.gl.activeTexture(this.gl.TEXTURE1);
            this.gl.u = this._createTexture();
            this.gl.uniform1i(this.gl.getUniformLocation(program, 'samplerU'), 1);

            this.gl.activeTexture(this.gl.TEXTURE2);
            this.gl.v = this._createTexture();
            this.gl.uniform1i(this.gl.getUniformLocation(program, 'samplerV'), 2);
        ...
        initVertexShader() {
            ...
            return this._compileShader(vertexShaderSource, this.gl.VERTEX_SHADER);
        },
        initFragmentShader() {
            ...
            return this._compileShader(fragmentShaderSource, this.gl.FRAGMENT_SHADER);
        },
